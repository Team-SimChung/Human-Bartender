...
namespace CodeStage.Maintainer.References
{
	...
	internal static class HierarchyScopeReferencesFinder
	{
		...
		public static HierarchyReferenceItem[] FindObjectsReferencesInHierarchy(Object[] objects, bool checkGameObjectsComponents, bool showResults = true)
		{
			...
			{
				SearchResultsStorage.HierarchyReferencesLastSearched = new long[0];
		...
		{
			var foundReferences = new Dictionary<long, Dictionary<ExactReferenceData, List<ExactReferenceData>>>(objects.Length);
		...
		private static ExactReferenceData ObjectToReferencingEntry(Object target)
		{
            ...
            var referencedSettings = new EntryAddSettings
            {
            	...
            	componentInstanceId = referencedObjectAsComponent != null ? CSInstanceIdTools.GetId(referencedObjectAsComponent) : -1,
            };
		...
		private static void BuildTree(Dictionary<long, Dictionary<ExactReferenceData, List<ExactReferenceData>>> foundReferences, List<HierarchyReferenceItem> results)
		{
			var id = 0;
		...
		private static IList<Object> GetFoundObjects(ICollection<Object> objects, List<HierarchyReferenceItem> results)
		{
			...
			{
				long componentInstanceId = -1;
				long objectInstanceId = -1;
				if (o is Component)
				{
					objectInstanceId = CSInstanceIdTools.GetId((o as Component).gameObject);
					componentInstanceId = CSInstanceIdTools.GetId(o);
				}
				else
				{
					objectInstanceId = CSInstanceIdTools.GetId(o);
				}
		...
		private static void SaveLastSearched(IList<Object> objects)
		{
			...
			var rootItems = new List<long>(resultsCount);
			for (var i = 0; i < resultsCount; i++)
			{
				...
				rootItems.Add(CSInstanceIdTools.GetId(result));
			}
