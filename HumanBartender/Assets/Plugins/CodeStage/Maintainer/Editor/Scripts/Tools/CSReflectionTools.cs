...
namespace CodeStage.Maintainer.Tools
{
	...
	public static class CSReflectionTools
	{
		...
		private static MethodInfo getMainAssetEntityIdMethodInfo;
		...
		/// <summary>
		/// AssetDatabase.GetMainAssetEntityId(string) is internal, not public, so it must be
		/// reached via reflection like GetMainAssetInstanceID above. It returns values in the
		/// correct EntityId space, unlike the legacy int-based GetMainAssetInstanceID whose
		/// numbering no longer matches Selection.entityIds/obj.GetEntityId() on Unity 6000.4+.
		/// </summary>
		public static MethodInfo GetGetMainAssetEntityIdMethodInfo()
		{
			if (getMainAssetEntityIdMethodInfo == null)
			{
				getMainAssetEntityIdMethodInfo = typeof(AssetDatabase).GetMethod("GetMainAssetEntityId", BindingFlags.NonPublic | BindingFlags.Static);
			}

			return getMainAssetEntityIdMethodInfo;
		}
#endif
