...
namespace CodeStage.Maintainer.Tools
{
	...
	internal static class CSPrefabTools
	{
		...
		public static void GetOverridenObjectsFromWholePrefabInstance(GameObject target, out long[] dirtyComponents)
		{
			var resultComponents = new HashSet<long>();
			...
			foreach (var objectOverride in objectOverrides)
			{
				...
				if (component != null)
				{
					resultComponents.Add(CSInstanceIdTools.GetId(component));
				}
			}
			...
			foreach (var addedComponent in addedComponents)
			{
				...
				{
					resultComponents.Add(CSInstanceIdTools.GetId(addedComponent.instanceComponent));
				}
			}
