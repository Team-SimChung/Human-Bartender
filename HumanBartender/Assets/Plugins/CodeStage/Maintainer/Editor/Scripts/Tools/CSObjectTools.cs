	...
	internal static class CSObjectTools
	{
		public static long GetUniqueObjectId(Object unityObject, bool recursiveCall = false)
		{
			...
			else
			{
				...
				if (id <= 0)
				{
					id = CSInstanceIdTools.GetId(unityObject);
				}
			}
		...
		public static long[] GetObjectsInstanceIDs(Object[] objects)
		{
			var instanceIds = new long[objects.Length];
			for (var i = 0; i < objects.Length; i++)
			{
				instanceIds[i] = CSInstanceIdTools.GetId(objects[i]);
			}

			return instanceIds;
		}

		public static Object[] GetObjectsFromInstanceIds(long[] objectInstanceIDs)
		{
			var objects = new List<Object>(objectInstanceIDs.Length);
			foreach (var id in objectInstanceIDs)
			{
				var o = CSInstanceIdTools.IdToObject(id);
				if (o != null)
				{
					objects.Add(o);
				}
